...
/** Astro `/blog/introduction` maps to the same post as the client's `introducing-filosign` entry. */
export function getBlogPost(slug: string): BlogPost {
	if (slug === "introduction") {
		const base = blogPosts["introducing-filosign"];
		return { ...base, id: "introduction" };
	}
	return blogPosts[slug] ?? blogPosts["introducing-filosign"];
}
